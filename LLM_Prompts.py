...
- **Cross-Border Displacement Inflows**: Consider whether conflict or other factors in neighboring countries could lead to new refugee or IDP inflows.
...
3.  **Risk Categories (H2):** For each risk area (Political, Armed Conflict, Natural Hazard, Cross-Border Displacement Inflows), create a level 2 heading (`##`), e.g., `## Political Risk`. Beneath each heading, write a short narrative paragraph summarizing the general situation for that risk.
...
5.  **Scenario Details:** Beneath each scenario heading, provide:
    ...
    * A bulleted list (`-`) with the following four items, using the exact bolded labels:
        ...
        * `- **Affected Population:**` [Provide total people and children affected, with rationale]
        * `- **Percentage of Population in Affected Area:**` [Provide percentage and context.]
        * `- **Potential Sectors for Assistance:**` [List relevant sectors, with description of the needs in each sector. Use a nested bulleted list if there are multiple.]
...
6.  **Sources (H2):** Conclude the entire report with a level 2 heading (`##`) titled `Sources Consulted`. **This section is mandatory.**
    ...
    * **CRITICAL RULE: You must not use placeholder text like '[Insert URL here]'. If you cannot find a specific source, omit it, but you must list at least two valid, real URLs.**
...
## Rules
1.  **CRITICAL RULE: You must ONLY include countries and scenarios that are explicitly present in the provided Source Text. Do not invent, hallucinate, or add information for any country not mentioned in the text.**
2.  **NO CONVERSATION:** Your output must be ONLY the final Markdown table. Do not provide any introduction, summary, explanation, or commentary. Under no circumstances should you ask for clarification or missing information.
3.  **DATA EXTRACTION:**
...
4.  **COMPLETENESS:** You must create a separate table row for every single scenario found in the source text. Do not omit, combine, or aggregate scenarios.
5.  **MISSING DATA:** If a specific data point for a scenario is not present in the text, leave that cell in the table blank. Do not write "N/A", "not available", or attempt to find the information yourself.
6.  **SORTING:** The final table must be sorted alphabetically by the `Country` column.
7.  **FORMATTING:** The generated Markdown table must be a single, contiguous block of text. There must be no blank lines between the header row, the separator line (`|---|...`), and the first data row.
"""
